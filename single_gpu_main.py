...
def parse_args():
    parser = argparse.ArgumentParser(description='U-Net Image Segmentation (Single GPU)')
...
def main():
    ...
    if args.eval:
        evaluate(model, val_loader, criterion, 0)
    else:
        for epoch in range(args.epochs):
            train_one_epoch(model, train_loader, criterion, optimizer, scheduler, epoch, 0)
            evaluate(model, val_loader, criterion, 0)
